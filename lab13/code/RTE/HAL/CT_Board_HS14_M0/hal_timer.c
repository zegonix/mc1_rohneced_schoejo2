...
#define CHECK_ARRAY_OUT_OF_BOUNDS_COMP_WRITE        0x04
...
void hal_timer_reset(reg_tim_t *timer)
{
    ...
    if(timer == TIM1 || timer == TIM8) {
        ...
        NVIC->ICER[irq_params.nvic_reg_nr] |= irq_params.nvic_irq_mask;
                       ...
                       hal_bool_t status)
{
    ...
    if (status == ENABLE) {
        timer->DIER |= irq;
        NVIC->ISER[irq_params.nvic_reg_nr] |= irq_params.nvic_irq_mask;
    } else {
        timer->DIER &= ~irq;
        NVIC->ICER[irq_params.nvic_reg_nr] |= irq_params.nvic_irq_mask;
    }
                                ...
                                uint32_t value)
{
    ...
    if (channel >= CHECK_ARRAY_OUT_OF_BOUNDS_COMP_WRITE) {
        return (-1);
    } else {
        /* convert pulse */
        if (timer != TIM2 && timer != TIM5){
