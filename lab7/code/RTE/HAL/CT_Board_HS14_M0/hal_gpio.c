                             ...
                             hal_gpio_output_t init)
{
    ...
    /* process af type */
    port->AFR[0] &= ~create_pattern_mask(init.pins, 0xf, 4u);
    port->AFR[0] |= create_pattern_mask(init.pins, af_mode, 4u);
    port->AFR[1] &= ~create_pattern_mask((init.pins >> 8), 0xf, 4u);
    port->AFR[1] |= create_pattern_mask((init.pins >> 8), af_mode, 4u);
}
    ...
    for (exti_line = 0u; exti_line < 16u; exti_line++) {
        if (pins & (0x1 << exti_line)) {
            ...
            if (status == ENABLE) {                
                ...
                SYSCFG->EXTICR[syscfg_bank] &= ~(0xf << syscfg_shift);
                SYSCFG->EXTICR[syscfg_bank] |= (exticr_mask << syscfg_shift);
                /* Unmask interrupt */
                EXTI->IMR |= (0x1 << exti_line);
                NVIC->ISER[nvic_bank] |= ((exti_line <  5u) ? (0x1 << (exti_line + NVIC_OFFSET_1_4)) :
                                          (exti_line < 10u) ? NVIC_OFFSET_5_9 : NVIC_OFFSET_10_15);
            } else {
                /* Mask interrupt */
                EXTI->IMR &= ~(0x1 << exti_line);
                NVIC->ICER[nvic_bank] |= ((exti_line <  5u) ? (0x1 << (exti_line + NVIC_OFFSET_1_4)) :
                                          (exti_line < 10u) ? NVIC_OFFSET_5_9 : NVIC_OFFSET_10_15);
            }
        }
    }
